...
export class ClassTeacherController {
  ...
  @Post('register')
  async createClassTeacher(
    @Body() body: ClassTeacherDto,
    @Res() response: Response,
  ) {
    if (
      !body.year ||
      body.class_id.length != 24 ||
      body.teacher_id.length != 24
    ) {
    ...
    this.classTeacherService
      .createClassTeacher(body)
      .then((res) => {
        if (res.success == 0) {
  ...
  async viewClassTeacherByClassId(
