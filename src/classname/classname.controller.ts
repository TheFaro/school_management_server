...
export class ClassnameController {
  ...
  @Post("register")
  async createClassname(@Body() body: ClassnameDto, @Res() response) {
    if (!body.name) {
    ...
    this.classnameService.addNewClass(body).then((res) => {
      if (res.success == 0) {
  ...
  async viewAllClasses(@Res() response) {
    this.classnameService.viewAllClassnames().then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  @Put("update/:id")
  async updateClassname(@Param("id") id: string, @Body() body: ClassnameDto, @Res() response) {
    ...
    this.classnameService.updateClassname(id, body).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  async deleteClass(@Param("id") id: string, @Res() response) {
    ...
    this.classnameService.deleteClassname(id).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
