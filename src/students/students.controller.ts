...
export class StudentsController {
  ...
  async createStudent(@Body() body: StudentsDto, @Res() response) {
    ...
    this.studentsService.createStudent(body).then((res) => {
      if (res.success == 0) {
  ...
  async getStudents(@Res() response) {
    this.studentsService.viewAllStudents().then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  async getOneStudent(@Param("id") id: string, @Res() response) {
    ...
    this.studentsService.viewStudentById(id).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  @Put("update/:id")
  async updateStudent(@Param("id") id: string, student: StudentsDto, @Res() response) {
    if (id.length != 24) {
      response.status(404).json({
        success: 0,
        message: "Required fields missing."
    ...
    this.studentsService.updateStudent(id, student).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  async deleteStudent(@Param("id") id: string, @Res() response) {
    ...
    this.studentsService.deleteStudent(id).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
