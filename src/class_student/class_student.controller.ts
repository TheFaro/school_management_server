...
export class ClassStudentController {
  ...
  async create(@Body() body: ClassStudentDto, @Res() response: Response) {
    if (body.classId.length != 24 && body.studentId.length != 24 && !body.year) {
    ...
    this.classStudentService.create(body).then((res) => {
      if (res.success == 0)
  ...
  async viewAll(@Res() response: Response) {
    this.classStudentService.viewAll().then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  async viewByYear(@Param("year") year: string, @Res() response: Response) {
    ...
    this.classStudentService.viewByYear(year).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  async viewByClassYear(@Param("class") classId: string, @Param("year") year: string, @Res() response: Response) {
    ...
    this.classStudentService.viewByClassAndYear(classId, year).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  @Put("update/:id")
  async update(@Param("id") id: string, @Body() body: ClassStudentDto, @Res() response: Response) {
    if (id.length != 24 && body.classId.length != 24 && body.studentId.length != 24 && body.year.length != 4) {
      response.status(404).json({
        success: 0,
        message: "Required fields missing."
      });
    ...
    this.classStudentService.update(id, body).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  async delete(@Param("id") id: string, @Res() response: Response) {
    ...
    this.classStudentService.delete(id).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
