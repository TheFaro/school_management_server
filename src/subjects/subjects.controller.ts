...
export class SubjectsController {
  ...
  async createSubject(@Body() body: SubjectsDto, @Res() response: Response) {
    ...
    this.subjectsService.createSubject(body).then((res) => {
      if (res.success == 0) {
  ...
  async viewAllSubject(@Res() response: Response) {
    this.subjectsService.viewAllSubjects().then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  async viewOneSubject(@Param("id") id: string, @Res() response: Response) {
    ...
    this.subjectsService.viewOneSubject(id).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  @Put("update/:id")
  async updateSubject(@Param("id") id: string, @Body() body: SubjectsDto, @Res() response: Response) {
    if (id.length != 24 && body.departmentId.length != 24 && !body.level && !body.name) {
      response.status(404).json({
        success: 0,
        message: "Required fields missing."
      });
    ...
    this.subjectsService.updateSubject(id, body).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  async deleteSubject(@Param("id") id: string, @Res() response: Response) {
    ...
    this.subjectsService.deleteSubject(id).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
