...
export class TeacherController {
  ...
  async createTeacher(@Body() body: TeacherDto, @Res() response: Response) {
    ...
    this.teacherService.createTeacher(body).then((res) => {
      if (res.success == 0) {
  ...
  async getAllTeachers(@Res() response: Response) {
    this.teacherService.viewAllTeachers().then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  @Put("update/:id")
  async updateTeacher(@Param("id") id: string, @Body() body: TeacherDto, @Res() response: Response) {
    if (id.length != 24) {
      response.status(404).json({
        success: 0,
        message: "Required fields missing."
    ...
    this.teacherService.updateTeacher(id, body).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
  ...
  async deleteTeacher(@Param("id") id: string, @Res() response: Response) {
    ...
    this.teacherService.deleteTeacher(id).then((res) => {
      if (!res) {
        response.status(400).json({
          success: 0,
